...
def get_next_splits():
    ...
    reverse_splits = list(
        filter(
            lambda split: split.ratio.startswith("1:")
            or split.ratio.startswith("1.00 -"),
            all_splits,
        )
    )
    ...
    reverse_splits = list(
        filter(lambda split: split.exchange not in exchange_blacklist, reverse_splits)
    )

    reverse_splits = list(
        filter(
            lambda split: datetime.today()
            <= datetime.strptime(split.effective_date, "%Y-%m-%d"),
            reverse_splits,
        )
    )

    reverse_splits = sorted(
        reverse_splits,
        key=lambda split: datetime.strptime(split.effective_date, "%Y-%m-%d"),
    )
