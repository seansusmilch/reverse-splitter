...
def schedule_jobs():
    research_job = scheduler.add_job(
        research.research_job,
        CronTrigger.from_crontab(CRON_RESEARCH),
        replace_existing=True,
    )
    newsletter_job = scheduler.add_job(
        send_newsletter.send_newsletter_job, CronTrigger.from_crontab(CRON_NEWSLETTER)
    )
    subscribe_job = scheduler.add_job(
        new_subs.add_new_subscribers_job,
        CronTrigger.from_crontab(CRON_ADD_NEW_SUBSCRIBERS),
    )
